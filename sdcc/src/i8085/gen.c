  ...
  switch (op1type)
          ...
          if (op2->aopu.aop_pairId == PAIR_BC || op2->aopu.aop_pairId == PAIR_DE)
            {
              /* ldax b / ldax d only ever load into a; stax b / stax d only
                 ...
                 - swapped == true (op1 was originally the memory side, op2
                   ...
                   functionally wrong build - worth spot-checking by
                   disassembly, not just a clean compile, if this is ever
                   touched). */
              if (form_out)
    ...
    case AOP_SFR:
      /* Writing to an output port ("out (n), a") is not a mov-family
         ...
         on a non-register aop should just always be false); left exactly
         as found rather than fixed, since it doesn't actually change this
         function's cost/branch behavior, only how it's tagged. Flagged for
         whoever next touches this function's cost accounting. */
      if (form_out)
    ...
    case AOP_EXSTK:
      ...
      if (form_out)
        wassertl (0, "AOP_EXSTK as a 16-bit ld_cost op1, with form_out set - real-emission reachability not yet confirmed, see comment");
      switch (op2type)
        {
        case AOP_IMMD:
        case AOP_LIT:
        ...
        case AOP_EXSTK:
          return (16);
        default:
          printf ("ld_cost op1: AOP_EXSTK, op2: %d\n", (int) (op2type));
          wassert (0);
        }
    case AOP_STK:
      // op1 == AOP_STK is dead for i8085 (omit_frame_ptr() always true -
      // see this function's other AOP_STK entry, the op1==AOP_REG/
      // op2==AOP_STK case above, for the full explanation). This whole
      // case, not just one nested arm, is unreachable - gated the same
      // way (only trips for emit_A_LD()'s real-emission callers).
      if (form_out)
        wassertl (0, "op1 == AOP_STK is dead for i8085 (omit_frame_ptr() always true)");
      switch (op2type)
        {
        case AOP_IMMD:
        case AOP_LIT:
        ...
        case AOP_HL:
          ...
          return (7);
        case AOP_IY: // dead for i8085 - no IY register exists on this CPU family.
          if (form_out) // see the AOP_STK comment above (op1 case) for why this is gated
            wassertl (0, "AOP_IY is dead for i8085 (no IY hardware)");
          if (count)
            {
              cost2 (4, 14);
    ...
    case AOP_HL:
      ...
      switch (op2type)
        ...
        case AOP_LIT:
          ...
          return (5);
        case AOP_STK:
          // op2 == AOP_STK is dead for i8085, same as every other
          // AOP_STK site in this function - kept (rather than merged
          // into AOP_HL/AOP_EXSTK below) only because its cost here
          // genuinely differs from theirs.
          if (form_out)
...
emit3Cost (enum asminst inst, const asmop *op1, int offset1, const asmop *op2, int offset2)
{
  ...
  switch (inst)
    ...
    case A_LD:
      ...
      return;
    // A_LDD/A_LDI are never passed to this function - the only site that
    // uses them, emit8080Ldi(), emits its own assembly/cost directly rather
    // than dispatching through here. A_LDF is never passed either: i8080/
    // i8085 have no far-pointer "ldf" instruction.
    case A_ADD:
    case A_ADC:
    case A_AND:
    case A_CP:
  ...
  switch (inst)
    ...
    case A_XOR:
      if (op2->type == AOP_LIT || op2->type == AOP_IMMD)
        ...
        {
          // op2 == AOP_STK is dead for i8085 (omit_frame_ptr() always
          // true) - kept (rather than merged into the else branch
          // below) only because its cost here genuinely differs.
          wassertl (0, "AOP_STK is dead for i8085 (omit_frame_ptr() always true)");
          cost2 (3, -1);
        }
      else
...
emit_A_LD (asmop *op1, int offset1, asmop *op2, int offset2)
{
  ...
  switch (form)
    ...
    case LDF_OUT_SPECIAL:
      /* Cost-only classifications: real LDA/STA/OUT emission goes through
         aopGet()'s own embedded AOP_DIR/AOP_SFR side effects (reads) or
         aopPut() (writes), not through emit3_o()'s generic path.
         ld_cost_form() still needs to classify these correctly for its
         cost-only callers (several exist, calling plain ld_cost() rather
         than emit3(A_LD, ...) - see ld_cost_form()'s own AOP_DIR/AOP_SFR
         comments), but no emit3(A_LD, ...) call site in this file is
         believed to reach here for real emission. Asserting rather than
         guessing at emission behavior never observed being exercised: if
         this fires, that belief was wrong and needs investigating, not
...
aopForSym (const iCode *ic, symbol *sym, bool result, bool requires_a)
{
  ...
  /* There is no IY on this port, so the address always goes in HL. */
  else
  ...
  for(; size; size--, offset1++, offset2++)
    {
      ...
      if ((aop1->type == AOP_DIR || aop1->type == AOP_HL) &&
        (aop2->type == AOP_DIR || aop2->type == AOP_HL) &&
...
requiresHL (const asmop *aop)
{
  switch (aop->type)
    {
    case AOP_IY: // dead for i8085 - no IY register exists on this CPU family.
      wassertl (0, "requiresHL: AOP_IY is dead for i8085 (no IY hardware)");
      return false;
    case AOP_HL:
    case AOP_EXSTK:
    case AOP_STL:
...
fetchPairLong (PAIR_ID pairId, asmop *aop, const iCode *ic, int offset)
{
  ...
  if (isLitWord (aop))
  ...
  else
    {
      ...
      /* 8080/8085 has no way to load DE/BC directly from a 16-bit
        ...
        (aop->size >= 2 || optimize.allow_unsafe_read))
        {
          if (pairId == PAIR_HL)
            ...
            /* 8080/8085: only "lhld" (load HL direct) exists - there is
               no "ld bc,(nn)"/"ld de,(nn)" equivalent at all (see
               genPointerGet's identical gap, gen.c ~13086, for the fuller
               comment and the empirical check backing "believed dead" -
               this site was found unguarded during that investigation,
               its sibling already had this tripwire). Live case
               (pairId == PAIR_HL) is handled above; non-HL is a loud
               tripwire, not a silent mistranslation. */
            wassertl (0, "fetchPairLong: literal-address pair read into non-HL pair - believed dead, see comment");
          if (pairId == PAIR_HL)
            cost2 (3, 16);
          else
            cost2 (4, 20);
      ...
      else if (pairId == PAIR_HL && (aop->type == AOP_HL || aop->type == AOP_EXSTK) && requiresHL (aop))
        {
...
setupPairFromSP (PAIR_ID id, int offset)
{
  ...
  if (offset < INT8MIN || offset > INT8MAX)
  ...
  else
    {
      ...
      /* Emitted directly here as "lxi h, #%d" / "dad sp" rather than going
         through emit3_o()'s A_LD dispatch: lxi's immediate-only nature
         means there is no register-vs-immediate ambiguity to resolve, so
...
static void pointPairToAop (PAIR_ID pairId, const asmop *aop, int offset)
{
  switch (aop->type)
    ...
    case AOP_EXSTK:
      ...
      if ((_G.pairs[pairId].last_type == AOP_EXSTK) && abs (_G.pairs[pairId].offset - abso) < (_G.preserveCarry ? 5 : 3))
    ...
    case AOP_EXSTK:
      {
        ...
        if ((_G.pairs[pairId].last_type == AOP_EXSTK) && abs (_G.pairs[pairId].offset - abso) <= 3)
          adjustPair (_pairs[pairId].name, &_G.pairs[pairId].offset, abso);
        else
          setupPairFromSP (pairId, abso + _G.stack.pushed);
        _G.pairs[pairId].offset = abso;
...
aopGet (asmop *aop, int offset, bool bit16)
{
  ...
  if (offset > (aop->size - 1) && aop->type != AOP_LIT)
              ...
              switch (offset)
                ...
                case 2:
                  if (aop->banked)
                    ...
                    dbuf_tprintf (&dbuf, "#0", aop->aopu.aop_immd);
                  break;
    ...
    /* "Banked"/16-bit I/O-port addressing has no 8080/8085 hardware
        ...
        case AOP_IY: // dead for i8085 - no IY register exists on this CPU family (same evidence as aopPut's AOP_IY case).
          wassertl (0, "aopGet: AOP_IY is dead for i8085 (no IY hardware)");
          break;

        case AOP_EXSTK:
        case AOP_STK:
          /* AOP_STK is never actually constructed - ralloc2.cc's
             omit_frame_ptr() unconditionally returns true, so newAsmop()
             always picks AOP_EXSTK over its AOP_STK sibling (see aopPut's
             own AOP_STK wassertl for the same proof). Both case labels are
             kept anyway (AOP_STK costs nothing to keep routing here, and
             stays a safe fallback if that proof is ever wrong). */
          pointPairToAop (PAIR_HL, aop, offset);
          dbuf_tprintf (&dbuf, "!*hl");
          break;
      ...
      if (aop->aopu.aop_pairId == PAIR_HL)
        {
          /* shiftIntoPair() (the only place that constructs an AOP_PAIRPTR)
             ...
             this file), not just BC/DE. A PAIR_HL AOP_PAIRPTR reaching this
             function without this arm would fall through to the BC/DE-only
             ternary below and silently emit "stax d" - a write through
             whatever DE happened to hold (stale, unrelated memory) instead
             of the real destination pointer in HL. Same "no mov m, m"
             hazard as the AOP_HL/
...
cheapMove (asmop *to, int to_offset, asmop *from, int from_offset, bool a_dead)
{
  ...
  if (from->type == AOP_IY || to->type == AOP_IY) // dead for i8085 - no IY register exists on this CPU family.
    ...
    (to->type == AOP_DIR || to->type == AOP_HL || to->type == AOP_EXSTK) && (from->type == AOP_HL || from->type == AOP_EXSTK) ||
...
commitPair (asmop *aop, PAIR_ID id, const iCode *ic, bool dont_destroy) // Obsolete. Replace uses by genMove or genMove_o.
{
  ...
  else if (!regalloc_dry_run && (aop->type == AOP_EXSTK) && !sp_offset)
    {
      emit2 ("inx sp");
      cost2 (1, 6);
      emit2 ("inx sp");
  ...
  /* PENDING: Verify this. */
  else if (id == PAIR_HL && requiresHL (aop) && aop->type != AOP_HL)
    {
  ...
  else
    {
      ...
      if (aop->type == AOP_HL && aop->size == 2 && id == PAIR_HL)
        {
...
/*-----------------------------------------------------------------*/
static void
genCopy (asmop *result, int roffset, asmop *source, int soffset, int sizex, bool a_dead, bool hl_dead, bool de_dead, bool f_dead)
{
  ...
  for (int i = 0; i < n && source->type == AOP_REG;)
    {
      ...
      if (assigned[i])
        ...
        aopInReg (source, soffset + i, HL_IDX) && hl_free)
        {
          ...
          emit3w (A_EX, ASMOP_DE, ASMOP_HL);
          spillPair (PAIR_HL);
          genCopy (result, roffset + i, ASMOP_DE, 0, 2, a_free, true, true, f_dead);
          if (!de_free)
            _pop (PAIR_DE);
          assigned[i] = true;
          assigned[i + 1] = true;
  ...
  for (int i = 0; i < n && result->type == AOP_REG;)
    {
      ...
      else if (i + 1 < n && !assigned[i + 1] && (source->type == AOP_EXSTK) && requiresHL (source) &&
...
genMove_o (asmop *result, int roffset, asmop *source, int soffset, int size, bool a_dead_global, bool hl_dead_global, bool de_dead_global, bool f_dead)
{
  ...
  emitDebug ("; genMove_o size %d result type %d source type %d a_dead %d hl_dead %d de_dead %d f_dead %d", size, result->type, source->type, a_dead_global, hl_dead_global, de_dead_global, f_dead);
  ...
  if ((result->type == AOP_REG || result->type == AOP_EXSTK) && (source->type == AOP_REG || source->type == AOP_EXSTK))
    {
      ...
      if (csize == 1 && aopInReg (result, roffset, A_IDX) && aopIsLitVal (source, soffset, 1, 0x00) && f_dead)
        ...
        genCopy (result, roffset, source, soffset, csize, a_dead_global, hl_dead_global, de_dead_global, f_dead);
      roffset += csize;
      size -= csize;
      ...
      genMove_o (result, roffset, ASMOP_ZERO, 0, size, a_dead, hl_dead, de_dead, f_dead);
      return;
    }
  ...
  for (int i = 0; i < size;)
    {
      ...
      if (source->type == AOP_STL && (soffset + i) >= 2)
      ...
      else if (source->type == AOP_STL)
        {
          ...
          if (i + soffset > 1)
            UNIMPLEMENTED;
          if ((_G.pairs[PAIR_HL].last_type == AOP_EXSTK) && abs (_G.pairs[PAIR_HL].offset - abso) < (f_dead ? 3 : 5))
          ...
          genMove_o (result, roffset + i, ASMOP_HL, soffset + i, size, a_dead, true, de_dead_global, f_dead);
          if (!hl_dead)
            {
              _pop (PAIR_HL);
              spillPair (PAIR_HL);
      ...
      if (i + 1 < size && getPairId_o(source, soffset + i) != PAIR_INVALID &&
        ...
        (result->type == AOP_DIR || result->type == AOP_HL) &&
        source->type == AOP_IMMD && hl_dead)
        {
          genMove_o (ASMOP_HL, 0, source, soffset + i, 2, a_dead, true, false, f_dead);
          genMove_o (result, roffset + i, ASMOP_HL, 0, 2, a_dead, true, false, f_dead);
          i += 2;
          continue;
        }
      ...
      if (f_dead && !zeroed_a && a_dead && source->regs[A_IDX] <= i &&
        ...
        size == 1 && result->type == AOP_HL && fetchLitPair (PAIR_HL, result, roffset + i, f_dead, true) && aopIsLitVal (source, soffset + i, 1, 0x00)))
        {
          emit3 (A_XOR, ASMOP_A, ASMOP_A);
          zeroed_a = true;
        }
...
/*--------------------------------------------------------------------------*/
static void
adjustStack (int n, bool af_free, bool bc_free, bool de_free, bool hl_free)
{
  if(n != 0)
  ...
  while (abs(n))
    {
      ...
      else if (n >= 2 && de_free && optimize.codeSize)
        {
  ...
  if (IS_FLOAT (type))
    {
      if (skipbyte != size - 1)
        UNIMPLEMENTED;
      emit8080SetRes (ASMOP_A, 0, 7, false, true);
      skipbyte = size - 1;
    }
  while (size--)
    if (size != skipbyte)
...
_castBoolean (const operand *right)
{
  ...
  if (right->aop->size == 1 && !aopInReg (right->aop, 0, A_IDX))
  ...
  else
    {
      ...
      // i8080/i8085 have no "neg" instruction, so negation is done via add/ld.
...
genIpush (const iCode *ic)
{
  ...
  if (size == 1 && IFFUNC_ISSMALLC (ftype)) // The SmallC calling convention pushes 8-bit parameters as 16-bit values.
    {
      ...
      if (size >= 2 && getPairId_o (ic->left->aop, size - 2) != PAIR_INVALID)
        {
         ...
         {
           genMove_o (ASMOP_H, 0, IC_LEFT (ic)->aop, size - 1, 1, a_free, h_free && l_free, d_free && e_free, true);
           emit3w (A_PUSH, ASMOP_HL, 0);
           emit2 ("inx sp");
           cost2 (1, 6);
           d = 1;
        ...
        {
          genMove_o (ASMOP_D, 0, IC_LEFT (ic)->aop, size - 1, 1, a_free, h_free && l_free, d_free && e_free, true);
          emit3w (A_PUSH, ASMOP_DE, 0);
          emit2 ("inx sp");
          cost2 (1, 6);
          d = 1;
        ...
        {
          genMove_o (ASMOP_B, 0, IC_LEFT (ic)->aop, size - 1, 1, a_free, h_free && l_free, d_free && e_free, true);
          emit3w (A_PUSH, ASMOP_BC, 0);
          emit2 ("inx sp");
          cost2 (1, 6);
          d = 1;
        ...
        {
          emit3w (A_PUSH, ASMOP_HL, 0);
          genMove_o (ASMOP_H, 0, ic->left->aop, size - 1, 1, a_free, h_free && l_free, d_free && e_free, true);
          emit2 ("xthl");
          cost2 (1, 16);
          spillPair (PAIR_HL);
          emit2 ("inx sp");
  ...
  if (bigreturn)
    {
      ...
      PAIR_ID pair = PAIR_HL;

      if (!hl_free)
        _push (PAIR_HL);
      aopOp (IC_RESULT (ic), ic, true, false);
      wassert (IC_RESULT (ic)->aop->type == AOP_EXSTK);
      fp_offset =
  ...
  if (ic->op == PCALL)
    {
      ...
      if (!jump &&
        ...
        {
          adjustStack (prestackadjust, a_free, bc_free, de_free, hl_free);
          if (!regalloc_dry_run)
      ...
      else if (aopInReg (ic->left->aop, 0, HL_IDX) || hl_free)
        {
          ...
          adjustStack (prestackadjust, a_not_parm, bc_not_parm, de_not_parm, false);
      ...
      // bc is the only 16-bit register guaranteed to be free here, even for __z88dk_fastcall.
      else if (bc_not_parm && (ic->left->aop->regs[B_IDX] < 0 && ic->left->aop->regs[C_IDX] < 0 || de_free)) // Try bc.
        {
          ...
          genMove (ASMOP_BC, IC_LEFT (ic)->aop, a_not_parm, hl_not_parm, de_not_parm);
          emit3w (A_PUSH, ASMOP_BC, 0);
          emit3 (A_RET, 0, 0);
          if (!regalloc_dry_run)
            _G.stack.pushed -= 2;
      ...
      else if (de_not_parm && (ic->left->aop->regs[D_IDX] < 0 && ic->left->aop->regs[E_IDX] < 0 || bc_free)) // Try de.
        {
          ...
          genMove (ASMOP_DE, IC_LEFT (ic)->aop, a_not_parm, hl_not_parm, true);
          emit3w (A_PUSH, ASMOP_DE, 0);
          emit3 (A_RET, 0, 0);
          if (!regalloc_dry_run)
            _G.stack.pushed -= 2;
  ...
  else
    {
      ...
      if (IFFUNC_ISBANKEDCALL (dtype))
      ...
      else
        {
          ...
          adjustStack (prestackadjust, false, bc_free, de_free, hl_free);
...
genFunction (const iCode * ic)
{
  ...
  if (IFFUNC_ISISR (sym->type))
    {
      ...
      /* Deliberately no "push iy"/"pop iy" here: i8080/i8085 have no IY
         ...
         __interrupt function's control flow, not just waste stack space. */
    }
  else
    {
  ...
  if (sym->stack)
    {
      adjustStack (-sym->stack, !i8085_IsParmInCall (sym->type, "a"), !i8085_IsParmInCall (sym->type, "c") && !i8085_IsParmInCall (sym->type, "v"), !i8085_IsParmInCall (sym->type, "e") && !i8085_IsParmInCall (sym->type, "d"), !i8085_IsParmInCall (sym->type, "l") && !i8085_IsParmInCall (sym->type, "h"));
      _G.stack.pushed = 0;
    }
...
genEndFunction (iCode *ic)
{
  ...
  if (IFFUNC_ISISR (sym->type))
    {
        {
          /* Deliberately no "pop iy" here - see the matching "push iy"
             comment in this function's prologue above. */
          emit2 ("pop de");
          emit2 ("pop bc");
          emit2 ("pop hl");
        }
  ...
  if (poststackadjust)
    {
      ...
      if (hl_free && !IFFUNC_ISISR (sym->type))
          ...
          // we also exactly know which registers we can trash
            {
              ...
              !aopRet (sym->type) || aopRet (sym->type)->regs[A_IDX] < 0, bc_free, de_free, false);
            }
      ...
      else if (bc_free || de_free)
        {
          ...
          adjustStack (poststackadjust, !aopRet (sym->type) || aopRet (sym->type)->regs[A_IDX] < 0, false, bc_free && de_free, hl_free);
  ...
  if (size <= 4 && !IS_STRUCT (operandType (IC_LEFT (ic))))
    {
      ...
      if (IC_LEFT (ic)->aop->type == AOP_LIT && size == 4 && !IS_FLOAT (IC_LEFT (ic)->aop->aopu.aop_lit->type) &&
        ...
        genMove (aopRet (currFunc->type), ic->left->aop, true, true, true);
    }
...
genPlus (iCode * ic)
{
  ...
  if (!maskedtopbyte && (isPair (IC_RIGHT (ic)->aop) || isPair (IC_LEFT (ic)->aop)) && getPairId (IC_RESULT (ic)->aop) == PAIR_HL)
    {
      ...
      if (left == PAIR_HL && right != PAIR_INVALID) // right != PAIR_IY dropped: getPairId() never returns PAIR_IY.
        ...
        {
          genMove_o (ASMOP_HL, 0, ic->left->aop, 0, 2, isRegDead (A_IDX, ic), true, isRegDead (DE_IDX, ic) && right != PAIR_DE, true); // was "isRegDead(IY_IDX,ic) && right != PAIR_IY" - both always true.
          emit3w (A_ADD, ASMOP_HL, ic->right->aop);
          goto release;
        }
      else if (left != PAIR_INVALID && left != PAIR_HL) // left != PAIR_IY dropped: getPairId() never returns PAIR_IY.
        {
          genMove_o (ASMOP_HL, 0, ic->right->aop, 0, 2, isRegDead (A_IDX, ic), true, isRegDead (DE_IDX, ic) && left != PAIR_DE, true); // was "isRegDead(IY_IDX,ic) && left != PAIR_IY" - both always true.
          emit3w (A_ADD, ASMOP_HL, ic->left->aop);
          goto release;
        }
      else if (left == PAIR_HL && (isPairDead (PAIR_DE, ic) || isPairDead (PAIR_BC, ic)))
        {
...

          {
            ...
            genMove (raop, ic->right->aop, false, false, false);
          }
      ...
      else if (right == PAIR_HL && (isPairDead (PAIR_DE, ic) || isPairDead (PAIR_BC, ic)))
        {
          ...
          {
            asmop *raop = isPairDead (PAIR_DE, ic) ? ASMOP_DE : ASMOP_BC;
            genMove (raop, leftop, false, false, false);
          }
          emit2 ("dad %s", _pairs[pair].name);
          cost2 (1, 10);
          goto release;
    ...
    (ic->right->aop->type == AOP_LIT || ic->right->aop->type == AOP_IMMD || ic->left->aop->type == AOP_IMMD && ic->right->aop->type == AOP_HL))
    {
      PAIR_ID extrapair = isPairDead (PAIR_DE, ic) ? PAIR_DE : PAIR_BC;
      genMove (ASMOP_HL, ic->left->aop, isRegDead (A_IDX, ic), true, isRegDead (DE_IDX, ic));
      genMove (extrapair == PAIR_DE ? ASMOP_DE : ASMOP_BC, ic->right->aop, isRegDead (A_IDX, ic), false, isRegDead (DE_IDX, ic));
  ...
  if (size == 2 && !maskedtopbyte &&
       ...
       requiresHL (rightop) && rightop->type != AOP_REG))))
    {
      ...
      if (save_de)
        _pop (PAIR_DE);
      genMove (IC_RESULT (ic)->aop, ASMOP_HL, isRegDead (A_IDX, ic), true, !save_de);
      goto release;
    }
  ...
  if (size == 4 && !maskedtopbyte &&
     ...
     (rightop->type == AOP_REG && rightop->regs[L_IDX] < 0 && rightop->regs[H_IDX] < 0 && requiresHL (leftop) && leftop->type != AOP_REG)))
    {
      ...
      adjustStack (-size, false, false, false, false);   /* alloc temp */
      ...
      adjustStack (size, false, false, false, false);   /* free temp */
      goto release;
    }
      ...
      if (!result_mem)
        {
          ...
          genMove (IC_RESULT (ic)->aop, &tmpaop, true, true, true);
          adjustStack (size, false, false, false, false);
        }
      if (save_de)
        _pop (PAIR_DE);
      if (save_bc)
  ...
  premoved = FALSE;
  if (size > 1 &&
    !(size == 2 && isPair (leftop) && rightop->type == AOP_LIT) && !(size == 2 && leftop->type == AOP_STL && isPairDead (PAIR_HL, ic))) // No need to setup if a single 16 bit addition is sufficient below.
    {
  ...
  for (i = 0, started = false; i < size;)
    {
      ...
      if (!maskedword && leftop->type == AOP_STL && !i && i + 1 < size && rightop->type == AOP_LIT && hl_dead)
        {
          ...
          spillPair (PAIR_HL);
          started = true;
          genMove_o (IC_RESULT (ic)->aop, 0, ASMOP_HL, 0, 2, true, true, de_dead, i + 2 == size);
          i += 2;
          continue;
        }
      else if (!maskedword && leftop->type == AOP_STL && !i && i + 1 < size && hl_dead && (size <= 2 || leftop->type != AOP_EXSTK /* (hl) would be pointed to result, overwritten by addition here */))
        {
          ...
          if (pair == PAIR_DE && !de_dead)
            _push (PAIR_DE);
          genMove (pair == PAIR_BC ? ASMOP_BC : ASMOP_DE, rightop, a_dead, true, de_dead);
          genMove (ASMOP_HL, leftop, true, true, de_dead && pair != PAIR_DE);
          emit2 ("dad %s", _pairs[pair].name);
          cost2 (1, 10);
          spillPair (PAIR_HL);
          started = true;
          if (pair == PAIR_DE && !de_dead)
            _pop (PAIR_DE);
          genMove_o (IC_RESULT (ic)->aop, 0, ASMOP_HL, 0, 2, a_dead, true, de_dead, i + 2 == size);
          i += 2;
          continue;
        }
      ...
      if (!maskedword && (!premoved || i) && !started && i == size - 2 && !i && isPair (rightop) && leftop->type == AOP_IMMD &&
        ...
        isPairDead (PAIR_HL, ic))
        {
          ...
          spillPair (PAIR_HL);
          started = true;
          genMove_o (IC_RESULT (ic)->aop, i, ASMOP_HL, 0, 2, a_dead, true, de_dead, true);
          i += 2;
        }
          ...
          if (aopInReg (rightop, i + 1, H_IDX) || aopInReg (rightop, i + 1, L_IDX))
            {
              ...
              genMove_o (ASMOP_HL, 0, leftop, i, 2, a_dead, true, de_dead, !started);
            }
          else
            {
              bool de_free = de_dead && ic->right->aop->regs[E_IDX] < i && ic->right->aop->regs[D_IDX] < i;
              genMove_o (ASMOP_HL, 0, ic->left->aop, i, 2, false, true, de_free, !started);
          ...
          if (aopInReg (rightop, i + 1, H_IDX) || aopInReg (rightop, i + 1, L_IDX))
            {
              ...
              genMove_o (ASMOP_HL, 0, leftop, i, 2, false, true, false, true);
            }
          else
            {
              bool de_free = de_dead && ic->right->aop->regs[E_IDX] < i && ic->right->aop->regs[D_IDX] < i;
              genMove_o (ASMOP_HL, 0, ic->left->aop, i, 2, false, true, de_free, true);
      ...
      // Conditional 8-bit inc.
        ...
        {
          genMove_o (ASMOP_HL, 0, leftop, i, 2, a_dead, true, false, !started);
          emit3w_o (started ? A_ADC : A_ADD, ASMOP_HL, 0, rightop, i);
          spillPair (PAIR_HL);
          genMove_o (ic->result->aop, i, ASMOP_HL, 0, 2, a_dead, true, false, i + 2 < size);
          started = true;
          i += 2;
        }
      else if (!maskedbyte && !premoved && !started && hl_dead2 &&
        ...
        {
          genMove_o (ASMOP_HL, 0, rightop, i, 2, a_dead, true, false, !started);
  ...
  if (!maskedtopbyte && size >= 1 &&
    ...
    requiresHL (right) && right->type != AOP_REG)
    {
      ...
      if (!result_mem)
        {
          /* move the temp (still at SP+0..) into the register result, then free it */
          genMove (result, &tmpaop, true, true, true);
          adjustStack (size, false, false, false, false);
        }
      if (save_de)
        _pop (PAIR_DE);
      ...
      if (ic->result->aop->type == AOP_REG)
        {
          ...
          if (aopInReg (ic->result->aop, 0, B_IDX) && IC_TRUE (ifx))
            {
      ...
      else
        {
          ...
          emit3 (A_DEC, ASMOP_A, 0);
          genMove_o (ic->result->aop, 0, ASMOP_A, 0, 1, true, isRegDead (HL_IDX, ic), isRegDead (DE_IDX, ic), false);
        }
      if (IC_TRUE (ifx))
...
genMultOneChar (const iCode * ic)
{
  ...
  // i8080/i8085 have no hardware multiply instruction, so this is always
  // the generic software shift-and-add multiply loop.
    {
...
/* i8080/i8085 have no hardware multiply instruction; genMultOneChar
   (the software shift-and-add fallback, used via the "else if
   (IC_RIGHT (ic)->aop->type != AOP_LIT)" arm just below) is the only
   multiply path on this port. */
/*-----------------------------------------------------------------*/
...
genCmp (operand * left, operand * right, operand * result, iCode * ifx, int sign, const iCode * ic)
{
  ...
  if (left->aop->type == AOP_CRY && right->aop->type == AOP_CRY)
  ...
  else
    {
      ...
      if (sign && size >= 1 && right->aop->type != AOP_LIT &&
      ...
      while (size)
        {
          ...
          if (started && !sign && aopIsLitVal (left->aop, offset, size, 0) && aopIsLitVal (right->aop, offset, size, 0)) // Skip leading zeroes.
            ...
            isPairDead (PAIR_HL, ic) &&
            (getPairId_o (left->aop, offset) == PAIR_HL || (left->aop->type == AOP_LIT || left->aop->type == AOP_IMMD || left->aop->type == AOP_HL) && right->aop->regs[L_IDX] < offset && right->aop->regs[H_IDX] < offset) &&
            ...
            (getPairId_o (left->aop, offset) == PAIR_HL || left->aop->type == AOP_LIT || left->aop->type == AOP_IMMD || left->aop->type == AOP_HL) &&
            (right->aop->type == AOP_LIT || right->aop->type == AOP_IMMD || right->aop->type == AOP_HL))
            {
              genMove_o (ASMOP_DE, 0, right->aop, offset, 2, isRegDead (A_IDX, ic), getPairId_o (left->aop, offset) != PAIR_HL, true, !offset);
              genMove_o (ASMOP_HL, 0, left->aop, offset, 2, isRegDead (A_IDX, ic), true, false, !offset);
              if (!started)
                emit3 (A_CP, ASMOP_A, ASMOP_A);
  ...
  if (max (left->aop->size, right->aop->size) > 1 && (couldDestroyCarry (left->aop) || couldDestroyCarry (right->aop)))
    {
      if ((requiresHL (IC_RESULT (ic)->aop) && IC_RESULT (ic)->aop->type != AOP_REG || requiresHL (left->aop) && left->aop->type != AOP_REG || requiresHL (right->aop) && right->aop->type != AOP_REG) &&
        ...
        UNIMPLEMENTED;
      else if (!(max (left->aop->size, right->aop->size) > 1 &&
                 left->aop->type != AOP_REG && requiresHL (left->aop) && left->aop->type != AOP_STL &&
                 right->aop->type != AOP_REG && requiresHL (right->aop) && right->aop->type != AOP_STL))
      ...
      while (size)
        {
          ...
          // Test for 0 can be done more efficiently using or
            ...
            (left->aop->type == AOP_REG) && skipbyte != offset + 1 &&
            ...
            {
              genMove_o (ASMOP_A, 0, left->aop, offset, 1, true, hl_dead, false, true);
      ...
      while (size--)
        {
          ...
          genMove_o (ASMOP_A, 0, left->aop, offset, 1, true, hl_dead, de_dead, true);
  ...
  if (result->aop->type == AOP_REG && left->aop->type != AOP_REG && right->aop->type != AOP_REG)
    {
      if (!requiresHL (right->aop) || (result->aop->regs[L_IDX] < 0 && result->aop->regs[H_IDX] < 0))
        { /* only if not (right requires HL and result use HL) */
          genMove (result->aop, left->aop, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), isPairDead (PAIR_DE, ic) && right->aop->regs[D_IDX] < 0 && right->aop->regs[E_IDX] < 0);
          left = result;
        }
    }
      ...
      if (left->aop->type == AOP_REG && right->aop->type == AOP_REG && // Try to use ld (nn), rr, etc.
      ...
      else if (aopIsLitVal (left->aop, i, 1, 0x00) && !pushed_a)
        {
          ...
          for(end = i; end < size && aopIsLitVal (left->aop, end, 1, 0x00); end++);
            genMove_o (result->aop, i, right->aop, i, end - i, a_free, hl_free, !isPairInUse (PAIR_DE, ic), true);
          if (result->aop->regs[A_IDX] >= i && result->aop->regs[A_IDX] < end)
            a_free = false;
          i = end;
          continue;
      ...
      else if (aopIsLitVal (right->aop, i, 1, 0x00) && !pushed_a)
        {
          ...
          for(end = i; end < size && aopIsLitVal (right->aop, end, 1, 0x00); end++);
            genMove_o (result->aop, i, left->aop, i, end - i, a_free, hl_free, !isPairInUse (PAIR_DE, ic), true);
          if (result->aop->regs[A_IDX] >= i && result->aop->regs[A_IDX] < end)
            a_free = false;
          i = end;
          continue;
      ...
      else if (aopIsLitVal (left->aop, i, 1, 0xff) || aopIsLitVal (right->aop, i, 1, 0xff))
        {
          ...
          for(end = i; end < size && (aopIsLitVal (left->aop, end, 1, 0xff) || aopIsLitVal (right->aop, end, 1, 0xff)); end++);
            genMove_o (result->aop, i, ASMOP_MONE, i, end - i, a_free, hl_free, !isPairInUse (PAIR_DE, ic), true);
          if (result->aop->regs[A_IDX] >= i && result->aop->regs[A_IDX] < end)
            a_free = false;
          i = end;
          continue;
...
shiftR2Left2Result (const iCode *ic, operand *left, int offl, operand *result, int offr, int shCount, int is_signed)
{
  ...
  if (IS_8085 && options.allow_undoc_inst && is_signed && shCount > 0 &&
    ...
    {
      genMove_o (ASMOP_HL, 0, left->aop, offl, 2, isRegDead (A_IDX, ic), true, isPairDead (PAIR_DE, ic), true);
      while (shCount-- > 0)
        {
          emit2 ("arhl");
          cost2 (1, 7);
        }
      genMove_o (result->aop, offr, ASMOP_HL, 0, 2, isRegDead (A_IDX, ic), true, isPairDead (PAIR_DE, ic), true);
      return;
    }
  if (result->aop != left->aop || offr != offl)
    genMove_o (result->aop, offr, left->aop, offl, 2, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), isPairDead (PAIR_DE, ic), true);
      ...
      if (s == 1)
        {
          ...
          if (size == 2 && aopInReg (left->aop, 0, HL_IDX) && isRegDead (HL_IDX, ic))
            rotaop = left->aop;
          genMove (rotaop, left->aop, true, isRegDead (HL_IDX, ic), isRegDead (DE_IDX, ic));
          cheapMove (ASMOP_A, 0, rotaop, size - 1, true);
          emit3 (A_RLA, 0, 0);
          if (requiresHL (rotaop) && rotaop->type != AOP_REG)
          ...
          genMove (result->aop, rotaop, true, isRegDead (HL_IDX, ic), isRegDead (DE_IDX, ic));
        }
      else
        {
          wassert (s == lbits - 1);
...
genRightShift (const iCode * ic)
{
  ...
  else if (isRegDead (B_IDX, ic) && result->aop->regs[B_IDX] < 0 &&
  ...
  if (!sameRegs (shiftop, left->aop) || shiftop->type == AOP_REG)
    {
      ...
      bool de_dead = isRegDead (DE_IDX, ic) && (countreg != E_IDX && countreg != D_IDX || shift_by_lit);
      if (!byteoffset)
        genMove_o (shiftop, 0, left->aop, soffset, size, !save_a, hl_dead, de_dead, true);
      else
        {
          if (save_a)
            _push (PAIR_AF);
          genMove_o (shiftop, 0, left->aop, soffset, size, true, hl_dead, de_dead, true);
  ...
  if (offset < rsize)
    {
      ...
      if (SPEC_USIGN (etype) || IS_BOOLEAN (etype))
        genMove_o (result->aop, offset, ASMOP_ZERO, 0, rsize, true, false, false, true);
      else
        {
...
genPointerGet (const iCode *ic)
{
  ...
  if (requiresHL (result->aop) && size > 1 && result->aop->type != AOP_REG)
    pair = PAIR_DE;
  ...
  if ((left->aop->type == AOP_IMMD || left->aop->type == AOP_LIT && !rightval) && size == 1 && aopInReg (result->aop, 0, A_IDX) &&
  ...
  else if (left->aop->type == AOP_STL && !bit_field && size <= 4)
    {
      ...
      genMove (result->aop, &saop, !surviving_a, isPairDead(PAIR_HL, ic), isPairDead(PAIR_DE, ic));
      goto release;
    }
  ...
  if (IS_8085 && options.allow_undoc_inst && !bit_field && !from_far && size == 2 &&
      ...
      (getPairId (left->aop) == PAIR_HL || getPairId (left->aop) == PAIR_DE))
    {
      ...
      spillPair (PAIR_HL);
      spillPair (PAIR_DE);
      genMove (result->aop, ASMOP_HL, isRegDead (A_IDX, ic), true, true);
      goto release;
    }
...
genPointerSet (iCode *ic)
{
  ...
  if (result->aop->type == AOP_STL && !bit_field && size <= 4)
    {
      ...
      genMove (&saop, right->aop, isRegDead (A_IDX, ic), isPairDead(PAIR_HL, ic), isPairDead(PAIR_DE, ic));
      goto release;
    }
  ...
  if (!to_far && !bit_field && isLitWord (result->aop) && size == 2 && offset == 0 && !sameRegs (result->aop, right->aop) &&
      ((right->aop->type == AOP_REG && getPairId (right->aop) == PAIR_HL) || (isLitWord (right->aop) && isPairDead (PAIR_HL, ic))) &&
      ...
      if ((!bit_field || blen >= 8) && aopIsLitVal (right->aop, offset, 1, 0x00) && zero_a)
      ...
      else
        {
          ...
          if (bit_field && blen < 8)
            ...
            {
              genMove_o (ASMOP_A, 0, right->aop, offset, 1, true, hl_dead, de_dead, true);
              zero_a = false;
...
genAssign (const iCode *ic)
{
  ...
  if (isPair (result->aop))
    ...
    right->aop->type == AOP_LIT && (result->aop->type == AOP_EXSTK) && (result->aop->aopu.aop_stk + offset + _G.stack.offset + (result->aop->aopu.aop_stk > 0 ? _G.stack.param_offset : 0) + _G.stack.pushed) == 0) // Use ex (sp), hl
    {
  ...
  else
    {
      ...
      if ((result->aop->type == AOP_REG || result->aop->type == AOP_EXSTK || result->aop->type == AOP_HL) && (right->aop->type == AOP_REG || right->aop->type == AOP_EXSTK || right->aop->type == AOP_LIT || right->aop->type == AOP_IMMD || right->aop->type == AOP_DIR || right->aop->type == AOP_HL))
        genMove (result->aop, right->aop, isRegDead (A_IDX, ic), isPairDead (PAIR_HL, ic), isPairDead (PAIR_DE, ic));
      else
        while (size--)
          {
            const bool hl_free = hl_dead &&
              ...
              (result->aop->regs[L_IDX] < 0 || result->aop->regs[L_IDX] >= offset) && (result->aop->regs[H_IDX] < 0 || result->aop->regs[H_IDX] >= offset);
            const bool save_hl = !hl_free && (requiresHL (right->aop) || requiresHL (result->aop));
...
genJumpTab (const iCode *ic)
{
  ...
  genMove (pair == PAIR_DE ? ASMOP_DE : ASMOP_BC, jtcond->aop, isRegDead (A_IDX, ic), true, isPairDead (PAIR_DE, ic));
  ...
  if (IS_BITINT (resulttype) && (SPEC_BITINTWIDTH (resulttype) % 8) && bitsForType (resulttype) < bitsForType (righttype))
    {
      if (!isRegDead (A_IDX, ic))
        _push (PAIR_AF), pushed_a = true;
      genMove (result->aop, right->aop, true, isPairDead (PAIR_HL, ic), isPairDead (PAIR_DE, ic));
...
setupForMemcpy (const iCode *ic, const operand *to, const operand *from, const operand *count)
{
  ...
  if (to->aop->type == AOP_REG && from->aop->type == AOP_REG)
  ...
  else if (to->aop->type == AOP_REG && count && count->aop->type == AOP_REG)
    {
      ...
      genMove (ASMOP_HL, from->aop, isRegDead (A_IDX, ic), true, false);
    }
  else if (from->aop->type == AOP_REG && count && count->aop->type == AOP_REG)
    {
      ...
      genMove (ASMOP_DE, to->aop, isRegDead (A_IDX, ic), false, true);
    }
  ...
  else
    {
      ...
      if (from->aop->type != AOP_REG || from->aop->aopu.aop_reg[0]->rIdx != E_IDX && from->aop->aopu.aop_reg[0]->rIdx != D_IDX && from->aop->aopu.aop_reg[1]->rIdx != E_IDX && from->aop->aopu.aop_reg[1]->rIdx != D_IDX)
        {
          ...
          genMove (ASMOP_DE, to->aop, a_free, from->aop->regs[L_IDX] < 0 && from->aop->regs[H_IDX] < 0, true);
          genMove (ASMOP_HL, from->aop, isRegDead (A_IDX, ic), true, false);
        }
      /* HL is free. Write it first. */
      else if (to->aop->type != AOP_REG || to->aop->aopu.aop_reg[0]->rIdx != L_IDX && to->aop->aopu.aop_reg[0]->rIdx != H_IDX && to->aop->aopu.aop_reg[1]->rIdx != L_IDX && to->aop->aopu.aop_reg[1]->rIdx != H_IDX)
        {
          ...
          genMove (ASMOP_HL, from->aop, a_free, true, to->aop->regs[E_IDX] < 0 && to->aop->regs[D_IDX] < 0);
          genMove (ASMOP_DE, to->aop, isRegDead (A_IDX, ic), false, true);
        }
