...
// (c) 2026 Neil Johnson <neil.johnson71@gmail.com>
//
...
static bool HLinst_ok(const assignment &a, unsigned short int i, const G_t &G, const I_t &I)
{
  ...
  /* Final fallback changed from return(true) to return(false) (#29):
     anything reaching this point has already failed every op-specific
     check above and isn't provably safe by any of them - "true" here
     was an unconditional "assume it's fine anyway" for whatever's left,
     the least justified of all the hatches this checkpoint removed.
     Confirmed via the same full-regression evidence as the rest of this
     function's changes (see the comments above). */
  return(false);
}
