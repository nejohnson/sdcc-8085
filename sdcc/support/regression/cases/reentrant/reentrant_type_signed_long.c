...
signed long sum(signed long a) __reentrant
{
  if(a>1) return a+sum(a-1);
  return 1;
}

signed long fact(signed long a) __reentrant
{
  if(a>1) return a*fact(a-1);
  return 1;
}

signed long div2n(signed long a, int n) __reentrant
{
  if(n>0) return div2n(a/2,n-1);
  return a;
}

signed long fib(signed long a) __reentrant
{
...
__getSuiteName(void)
{
  return "reentrant_type_signed_long";
}
