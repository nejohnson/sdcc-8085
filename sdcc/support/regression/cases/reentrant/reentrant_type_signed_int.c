...
signed int sum(signed int a) __reentrant
{
  if(a>1) return a+sum(a-1);
  return 1;
}

signed int fact(signed int a) __reentrant
{
  if(a>1) return a*fact(a-1);
  return 1;
}

signed int div2n(signed int a, int n) __reentrant
{
  if(n>0) return div2n(a/2,n-1);
  return a;
}

signed int fib(signed int a) __reentrant
{
...
__getSuiteName(void)
{
  return "reentrant_type_signed_int";
}
