...
unsigned char sum(unsigned char a) __reentrant
{
  if(a>1) return a+sum(a-1);
  return 1;
}

unsigned char fact(unsigned char a) __reentrant
{
  if(a>1) return a*fact(a-1);
  return 1;
}

unsigned char div2n(unsigned char a, int n) __reentrant
{
  if(n>0) return div2n(a/2,n-1);
  return a;
}

unsigned char fib(unsigned char a) __reentrant
{
...
__getSuiteName(void)
{
  return "reentrant_type_unsigned_char";
}
