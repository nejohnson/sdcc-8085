...
unsigned long sum(unsigned long a) __reentrant
{
  if(a>1) return a+sum(a-1);
  return 1;
}

unsigned long fact(unsigned long a) __reentrant
{
  if(a>1) return a*fact(a-1);
  return 1;
}

unsigned long div2n(unsigned long a, int n) __reentrant
{
  if(n>0) return div2n(a/2,n-1);
  return a;
}

unsigned long fib(unsigned long a) __reentrant
{
...
__getSuiteName(void)
{
  return "reentrant_type_unsigned_long";
}
