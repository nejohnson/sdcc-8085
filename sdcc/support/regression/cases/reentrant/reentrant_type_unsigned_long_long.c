...
unsigned long long sum(unsigned long long a) __reentrant
{
  if(a>1) return a+sum(a-1);
  return 1;
}

unsigned long long fact(unsigned long long a) __reentrant
{
  if(a>1) return a*fact(a-1);
  return 1;
}

unsigned long long div2n(unsigned long long a, int n) __reentrant
{
  if(n>0) return div2n(a/2,n-1);
  return a;
}

unsigned long long fib(unsigned long long a) __reentrant
{
...
__getSuiteName(void)
{
  return "reentrant_type_unsigned_long_long";
}
