...
unsigned int sum(unsigned int a) __reentrant
{
  if(a>1) return a+sum(a-1);
  return 1;
}

unsigned int fact(unsigned int a) __reentrant
{
  if(a>1) return a*fact(a-1);
  return 1;
}

unsigned int div2n(unsigned int a, int n) __reentrant
{
  if(n>0) return div2n(a/2,n-1);
  return a;
}

unsigned int fib(unsigned int a) __reentrant
{
...
__getSuiteName(void)
{
  return "reentrant_type_unsigned_int";
}
