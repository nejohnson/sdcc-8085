...
{
	volatile counttype tmp = 33;
	return(tmp);
}

void testleftshift(void)
{
#if 64 <= MAXWIDTH && 33 < 64 && 1 == 0
	...
	ASSERT (result == (OPERAND0 << 33));

	operand = setoperand1();
	result = operand << count;
	ASSERT (result == (OPERAND1 << 33));
#endif
}

void testrightshift(void)
{
#if 64 <= MAXWIDTH && 33 < 64 && 1 == 1
	static bitinttype operand, result;
	auto counttype count;
...
__getSuiteName(void)
{
  return "bitintshift_width_64_count_33_varstorage_static_countstorage_auto_direction_1";
}
