...
{
	volatile counttype tmp = 16;
	return(tmp);
}

void testleftshift(void)
{
#if 63 <= MAXWIDTH && 16 < 63 && 1 == 0
	...
	ASSERT (result == (OPERAND0 << 16));

	operand = setoperand1();
	result = operand << count;
	ASSERT (result == (OPERAND1 << 16));
#endif
}

void testrightshift(void)
{
#if 63 <= MAXWIDTH && 16 < 63 && 1 == 1
	static bitinttype operand, result;
	auto counttype count;
...
__getSuiteName(void)
{
  return "bitintshift_width_63_count_16_varstorage_static_countstorage_auto_direction_1";
}
