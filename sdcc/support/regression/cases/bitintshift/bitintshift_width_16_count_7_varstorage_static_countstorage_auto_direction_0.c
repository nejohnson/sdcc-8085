...
{
	volatile counttype tmp = 7;
	return(tmp);
}

void testleftshift(void)
{
#if 16 <= MAXWIDTH && 7 < 16 && 0 == 0
	...
	ASSERT (result == (OPERAND0 << 7));

	operand = setoperand1();
	result = operand << count;
	ASSERT (result == (OPERAND1 << 7));
#endif
}

void testrightshift(void)
{
#if 16 <= MAXWIDTH && 7 < 16 && 0 == 1
	static bitinttype operand, result;
	auto counttype count;
...
__getSuiteName(void)
{
  return "bitintshift_width_16_count_7_varstorage_static_countstorage_auto_direction_0";
}
