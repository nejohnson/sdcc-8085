...
{
	volatile counttype tmp = 8;
	return(tmp);
}

void testleftshift(void)
{
#if 16 <= MAXWIDTH && 8 < 16 && 0 == 0
	...
	ASSERT (result == (OPERAND0 << 8));

	operand = setoperand1();
	result = operand << count;
	ASSERT (result == (OPERAND1 << 8));
#endif
}

void testrightshift(void)
{
#if 16 <= MAXWIDTH && 8 < 16 && 0 == 1
	static bitinttype operand, result;
	auto counttype count;
...
__getSuiteName(void)
{
  return "bitintshift_width_16_count_8_varstorage_static_countstorage_auto_direction_0";
}
