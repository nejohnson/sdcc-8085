...
{
	volatile counttype tmp = 40;
	return(tmp);
}

void testleftshift(void)
{
#if 64 <= MAXWIDTH && 40 < 64 && 1 == 0
	static bitinttype operand, result;
	auto counttype count;

	count = setcount();

	operand = setoperand0();
	result = operand << count;
	ASSERT (result == (OPERAND0 << 40));

	operand = setoperand1();
	result = operand << count;
	ASSERT (result == (OPERAND1 << 40));
#endif
}

void testrightshift(void)
{
#if 64 <= MAXWIDTH && 40 < 64 && 1 == 1
	static bitinttype operand, result;
	auto counttype count;
...
__getSuiteName(void)
{
  return "bitintshift_width_64_count_40_varstorage_static_countstorage_auto_direction_1";
}
