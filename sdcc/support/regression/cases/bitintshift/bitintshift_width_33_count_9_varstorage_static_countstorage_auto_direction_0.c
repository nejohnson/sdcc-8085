...
{
	volatile counttype tmp = 9;
	return(tmp);
}

void testleftshift(void)
{
#if 33 <= MAXWIDTH && 9 < 33 && 0 == 0
	...
	ASSERT (result == (OPERAND0 << 9));

	operand = setoperand1();
	result = operand << count;
	ASSERT (result == (OPERAND1 << 9));
#endif
}

void testrightshift(void)
{
#if 33 <= MAXWIDTH && 9 < 33 && 0 == 1
	static bitinttype operand, result;
	auto counttype count;
...
__getSuiteName(void)
{
  return "bitintshift_width_33_count_9_varstorage_static_countstorage_auto_direction_0";
}
