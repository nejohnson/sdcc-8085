...
{
	volatile counttype tmp = 47;
	return(tmp);
}

void testleftshift(void)
{
#if 63 <= MAXWIDTH && 47 < 63 && 0 == 0
	...
	ASSERT (result == (OPERAND0 << 47));

	operand = setoperand1();
	result = operand << count;
	ASSERT (result == (OPERAND1 << 47));
#endif
}

void testrightshift(void)
{
#if 63 <= MAXWIDTH && 47 < 63 && 0 == 1
	static bitinttype operand, result;
	auto counttype count;
...
__getSuiteName(void)
{
  return "bitintshift_width_63_count_47_varstorage_static_countstorage_auto_direction_0";
}
