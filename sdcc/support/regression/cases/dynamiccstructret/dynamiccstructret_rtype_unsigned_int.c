...
struct s1 fret1 (unsigned int p) __dynamicc
{
  struct s1 s;
  s.i = p;
  return s;
...
struct s2 fret2 (unsigned int p) __dynamicc
{
  struct s2 s;
  s.i = p;
  s.c = 'A';
...
void testDynamicC(void)
{
  ...
  fret1 (VAL2);
  s1 = fret1 (VAL1);
  ASSERT (s1.i == (unsigned int)VAL1);
  s2 = fret2 (VAL2);
  ASSERT (s2.i == (unsigned int)VAL2);
  ASSERT (s2.c == 'A');
  s2 = fret3 (s1);
  ASSERT (s2.i == (unsigned int)VAL1);
  ASSERT (s2.c == 'B');
#endif
#endif
}
...
__getSuiteName(void)
{
  return "dynamiccstructret_rtype_unsigned_int";
}
