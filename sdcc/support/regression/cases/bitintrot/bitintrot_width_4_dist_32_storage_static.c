...
#if __SDCC_BITINT_MAXWIDTH >= 4 && (4 <= 8 || !defined(__SDCC_pdk13) && !defined(__SDCC_pdk14)) && (4 <= 16 || !defined(__SDCC_pdk15)) // Lack of memory for pdk. TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
typedef unsigned _BitInt(4) bitinttype;
#define WIDTH 4
#else
...
__getSuiteName(void)
{
  return "bitintrot_width_4_dist_32_storage_static";
}
