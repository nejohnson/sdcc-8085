...
#if __SDCC_BITINT_MAXWIDTH >= 8 && (8 <= 8 || !defined(__SDCC_pdk13) && !defined(__SDCC_pdk14)) && (8 <= 16 || !defined(__SDCC_pdk15)) // Lack of memory for pdk. TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
typedef unsigned _BitInt(8) bitinttype;
#define WIDTH 8
#else
...
__getSuiteName(void)
{
  return "bitintrot_width_8_dist_20_storage_static";
}
