...
#if __SDCC_BITINT_MAXWIDTH >= 6 && (6 <= 8 || !defined(__SDCC_pdk13) && !defined(__SDCC_pdk14)) && (6 <= 16 || !defined(__SDCC_pdk15)) // Lack of memory for pdk. TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
typedef unsigned _BitInt(6) bitinttype;
#define WIDTH 6
#else
...
__getSuiteName(void)
{
  return "bitintrot_width_6_dist_16_storage_static";
}
