...
#if __SDCC_BITINT_MAXWIDTH >= 32 && (32 <= 8 || !defined(__SDCC_pdk13) && !defined(__SDCC_pdk14)) && (32 <= 16 || !defined(__SDCC_pdk15)) // Lack of memory for pdk. TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
typedef unsigned _BitInt(32) bitinttype;
#define WIDTH 32
#else
...
testRot (void)
{
	volatile static bitinttype t1 = TESTVECT1;
	volatile static bitinttype t2 = TESTVECT2;
...
__getSuiteName(void)
{
  return "bitintrot_width_32_dist_32_storage_static";
}
