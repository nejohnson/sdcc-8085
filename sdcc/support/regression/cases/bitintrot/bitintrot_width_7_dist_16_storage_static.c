...
#if __SDCC_BITINT_MAXWIDTH >= 7 && (7 <= 8 || !defined(__SDCC_pdk13) && !defined(__SDCC_pdk14)) && (7 <= 16 || !defined(__SDCC_pdk15)) // Lack of memory for pdk. TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
typedef unsigned _BitInt(7) bitinttype;
#define WIDTH 7
#else
...
__getSuiteName(void)
{
  return "bitintrot_width_7_dist_16_storage_static";
}
