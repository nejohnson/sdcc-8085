...
unsigned long fret (void) __dynamicc
{
  return ((unsigned long)VAL);
}

unsigned long fpar (unsigned long p0, unsigned long p1) __dynamicc
{
  return (p0 + p1);
}
...
{
  ASSERT (fret () == (unsigned long)VAL);
  ASSERT (frameptruser (1, 2, 3, 4) == 1 + 2 + 3 + 4 + (int)(unsigned long)VAL);
...
__getSuiteName(void)
{
  return "dynamic_rtype_unsigned_long";
}
