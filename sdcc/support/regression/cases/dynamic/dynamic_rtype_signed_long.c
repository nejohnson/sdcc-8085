...
signed long fret (void) __dynamicc
{
  return ((signed long)VAL);
}

signed long fpar (signed long p0, signed long p1) __dynamicc
{
  return (p0 + p1);
}
...
{
  ASSERT (fret () == (signed long)VAL);
  ASSERT (frameptruser (1, 2, 3, 4) == 1 + 2 + 3 + 4 + (int)(signed long)VAL);
...
__getSuiteName(void)
{
  return "dynamic_rtype_signed_long";
}
