...
unsigned char fret (void) __dynamicc
{
  return ((unsigned char)VAL);
}

unsigned char fpar (unsigned char p0, unsigned char p1) __dynamicc
{
  return (p0 + p1);
}
...
{
  ASSERT (fret () == (unsigned char)VAL);
  ASSERT (frameptruser (1, 2, 3, 4) == 1 + 2 + 3 + 4 + (int)(unsigned char)VAL);
...
__getSuiteName(void)
{
  return "dynamic_rtype_unsigned_char";
}
