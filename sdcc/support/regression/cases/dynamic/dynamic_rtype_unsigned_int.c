...
unsigned int fret (void) __dynamicc
{
  return ((unsigned int)VAL);
}

unsigned int fpar (unsigned int p0, unsigned int p1) __dynamicc
{
  return (p0 + p1);
}
...
{
  ASSERT (fret () == (unsigned int)VAL);
  ASSERT (frameptruser (1, 2, 3, 4) == 1 + 2 + 3 + 4 + (int)(unsigned int)VAL);
...
__getSuiteName(void)
{
  return "dynamic_rtype_unsigned_int";
}
