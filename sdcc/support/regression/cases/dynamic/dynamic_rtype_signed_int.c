...
signed int fret (void) __dynamicc
{
  return ((signed int)VAL);
}

signed int fpar (signed int p0, signed int p1) __dynamicc
{
  return (p0 + p1);
}
...
{
  ASSERT (fret () == (signed int)VAL);
  ASSERT (frameptruser (1, 2, 3, 4) == 1 + 2 + 3 + 4 + (int)(signed int)VAL);
...
__getSuiteName(void)
{
  return "dynamic_rtype_signed_int";
}
