...
__far unsigned long i;
__far unsigned long ii = 0x55;
__far unsigned long j;
const __far unsigned long ci = 0xaa;

unsigned long readfarptr(__far unsigned long *p)
{
	return(*p);
}

void writefarptr(__far unsigned long *p, unsigned long i)
{
	*p = i;
}
...
void testFarPtr(void)
{
	...
	ASSERT(readfarptr((__far unsigned long *)(&k)) == 23);
	writefarptr((__far unsigned long *)(&k), 42);
	ASSERT(k == 42);
	__far unsigned long *volatile p = (__far unsigned long *)(&k);
	ASSERT((unsigned long *)p == &k);
	ASSERT((unsigned long *)p == (__far unsigned long *)(&k));
#endif
}
...
__getSuiteName(void)
{
  return "far_rabbits_type_unsigned_long";
}
