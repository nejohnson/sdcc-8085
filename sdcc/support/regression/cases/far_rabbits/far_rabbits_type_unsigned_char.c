...
__far unsigned char i;
__far unsigned char ii = 0x55;
__far unsigned char j;
const __far unsigned char ci = 0xaa;

unsigned char readfarptr(__far unsigned char *p)
{
	return(*p);
}

void writefarptr(__far unsigned char *p, unsigned char i)
{
	*p = i;
}
...
void testFarPtr(void)
{
	...
	ASSERT(readfarptr((__far unsigned char *)(&k)) == 23);
	writefarptr((__far unsigned char *)(&k), 42);
	ASSERT(k == 42);
	__far unsigned char *volatile p = (__far unsigned char *)(&k);
	ASSERT((unsigned char *)p == &k);
	ASSERT((unsigned char *)p == (__far unsigned char *)(&k));
#endif
}
...
__getSuiteName(void)
{
  return "far_rabbits_type_unsigned_char";
}
