...
#if __SDCC_BITINT_MAXWIDTH >= 17 && (17 <= 40 || !defined(__SDCC_pdk13)) // Lack of memory for pdk13. TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
...
__runSuite(void)
{
  __prints("Running testWideBitIntBitField\n");
  testWideBitIntBitField();
}
...
__getSuiteName(void)
{
  return "widebitintbitfield_width_17_sign_unsigned";
}
