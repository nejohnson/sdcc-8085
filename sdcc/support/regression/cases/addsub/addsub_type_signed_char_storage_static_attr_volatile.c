...
signed char add_func(signed char i)
{
	return(i + (5ul << 16));
}
...
testAdd(void)
{
  ...
  result = left + right;
  ASSERT(result == (signed char)(0x5500 + 0x0a00));
  ...
  ASSERT(result == (signed char)(0x550000ul + 0x0a0000ul));

  ASSERT(add_func(0) == (signed char)(5ul << 16));
#endif
}
...
__getSuiteName(void)
{
  return "addsub_type_signed_char_storage_static_attr_volatile";
}
