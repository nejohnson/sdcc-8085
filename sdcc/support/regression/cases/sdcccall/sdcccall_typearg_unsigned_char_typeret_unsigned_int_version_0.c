...
unsigned int f(unsigned char i) SDCCCALL
{
	return ++i;
}

unsigned int g(unsigned char i, unsigned int j) SDCCCALL
{
	return i + j;
}

#ifndef __SDCC_pic16 // TODO: enable when pic16 supports long long
unsigned int h(unsigned char i, unsigned int j, unsigned long long k) SDCCCALL
{
	return i + j + k;
}
#endif

unsigned int f_wrap_default(unsigned char i)
{
	return f(i);
}

unsigned int f_wrap_explicit(unsigned char i) SDCCCALL
{
	return f(i);
}
...
__getSuiteName(void)
{
  return "sdcccall_typearg_unsigned_char_typeret_unsigned_int_version_0";
}
