/** A test based on the benchmark and its self-test
    ...
    mem_method: MEM_STACK, MEM_STATIC
 */
...
#if defined(__SDCC_pdk13)  || defined(__SDCC_pdk14)  || defined(__SDCC_pdk15)  || defined(__SDCC_pdk16) || defined(__SDCC_pic14) || defined(__SDCC_pic16) // We need at least 2.5 KB of memory, which no pdk port has.
#define LACK_OF_MEMORY
#endif

#if defined(__SDCC_hc08) || defined(__SDCC_s08) || defined(__SDCC_mos6502) || defined(__SDCC_mos65c02) || defined(__SDCC_mcs51) // Looks like we run out of space for local variables even with MEM_STATIC - this should improve when we have Chaitin-style allocation for local vairables not on the stack (RFE #958), any maybe some other optimizations.
#define LACK_OF_MEMORY
#endif
...
#endif
#if HAS_PRINTF
#define ee_printf(...) /* deviation from benchmark - no console output */
#endif
...
__getSuiteName(void)
{
  return "coremark_mem_method_MEM_STATIC";
}
