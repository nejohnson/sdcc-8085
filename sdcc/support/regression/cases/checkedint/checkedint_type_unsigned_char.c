...
unsigned char add_func(unsigned char i, unsigned char j)
{
	return(i + j);
}

unsigned char sub_func(unsigned char i, unsigned char j)
{
	return(i - j);
}

unsigned char mul_func(unsigned char i, unsigned char j)
{
	return(i * j);
}
#endif
...
__getSuiteName(void)
{
  return "checkedint_type_unsigned_char";
}
