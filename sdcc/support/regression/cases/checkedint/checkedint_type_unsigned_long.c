...
unsigned long add_func(unsigned long i, unsigned long j)
{
	return(i + j);
}

unsigned long sub_func(unsigned long i, unsigned long j)
{
	return(i - j);
}

unsigned long mul_func(unsigned long i, unsigned long j)
{
	return(i * j);
}
#endif
...
__getSuiteName(void)
{
  return "checkedint_type_unsigned_long";
}
