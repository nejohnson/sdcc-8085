...
#if __SDCC_BITINT_MAXWIDTH >= 9 // TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
#if 9 <= 40 || !defined (__SDCC_pdk14) // Lack of memory

typedef _BitInt(9) bitinttype;

typedef unsigned _BitInt(9) ubitinttype;
...
void testBitIntArith(void)
{
...
#if __SDCC_BITINT_MAXWIDTH >= 9 // TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
#if (9 <= 40 || !defined (__SDCC_pdk14)) && (9 <= 48 || !defined (__SDCC_pdk15)) // Lack of memory
...
#if 9 >= 8 // Signed overflow is undefined behaviour	
...
#if 9 >= 11 // Signed overflow is undefined behaviour
	ASSERT(a * b == (bitinttype)(23) * (bitinttype)(-42));
#endif
	...
	ua = 1ull << (9 - 1);
	ASSERT(ua / 4 == (1ull << (9 - 1)) / 4ull); // Underlying issue of bug #3855: a signed division was used for some unsigned _BitInt divisions.
#endif
#endif
#endif
#endif
