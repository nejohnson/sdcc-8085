...
#if __SDCC_BITINT_MAXWIDTH >= 8 // TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
#if 8 <= 40 || !defined (__SDCC_pdk14) // Lack of memory

typedef _BitInt(8) bitinttype;
...
void testBitIntArith(void)
{
...
#if __SDCC_BITINT_MAXWIDTH >= 8 // TODO: When we can regression-test in --std-c23 mode, use the standard macro from limits.h instead!
#if (8 <= 40 || !defined (__SDCC_pdk14)) && (8 <= 48 || !defined (__SDCC_pdk15)) // Lack of memory
...
#if 8 >= 8 // Signed overflow is undefined behaviour	
...
#if 8 >= 11 // Signed overflow is undefined behaviour
	ASSERT(a * b == (bitinttype)(23) * (bitinttype)(-42));
#endif
	...
	ua = 1ull << (8 - 1);
	ASSERT(ua / 4 == (1ull << (8 - 1)) / 4ull); // Underlying issue of bug #3855: a signed division was used for some unsigned _BitInt divisions.
#endif
#endif
#endif
#endif
