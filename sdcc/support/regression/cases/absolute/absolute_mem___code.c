...
__code volatile TestStruct __at(ADDRESS(BC)) TestVar = {0,0};
__code char __at(ADDRESS(B7)) u;
__code char __at(ADDRESS(B7)) x = 'x';
__code char __at(ADDRESS(B9)) y = 'y';
__code int __at(ADDRESS(B0)) k = 0x1234;

char * __code __at(ADDRESS(A0)) pc = NULL;

typedef char *Pointer;
__code Pointer __at(ADDRESS(A4)) p = NULL;

typedef char Array[2];
__code Array __at(ADDRESS(A8)) a = {0};

typedef void (* const Function)(void);
__code Function __at(ADDRESS(AC)) f = NULL;
#endif
...
testAbsolute(void)
{
  ...
  static __code char __at(ADDRESS(B6)) s = 's';
