...
#ifdef __SDCC_stm8
unsigned char f(unsigned int i) __raisonance
{
	return ++i;
}

unsigned char g(unsigned int i, unsigned int j) __raisonance
{
	return i + j;
}

unsigned char h(unsigned int i, unsigned int j, unsigned long long k) __raisonance
{
	return i + j + k;
}
...
unsigned char f_wrap___raisonance(unsigned int i) __raisonance
{
	return f(i);
}
#endif
...
__getSuiteName(void)
{
  return "stm8-conventions_typearg_unsigned_int_typeret_unsigned_char_convention___raisonance";
}
