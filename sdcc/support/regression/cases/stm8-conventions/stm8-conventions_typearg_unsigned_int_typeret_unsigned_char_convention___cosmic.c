...
#ifdef __SDCC_stm8
unsigned char f(unsigned int i) __cosmic
{
	return ++i;
}

unsigned char g(unsigned int i, unsigned int j) __cosmic
{
	return i + j;
}

unsigned char h(unsigned int i, unsigned int j, unsigned long long k) __cosmic
{
	return i + j + k;
}
...
unsigned char f_wrap___cosmic(unsigned int i) __cosmic
{
	return f(i);
}
#endif
...
testCallingConvention(void)
{
  ...
  ASSERT(f_wrap___cosmic(23) == (unsigned char)24);
#endif
}
...
__getSuiteName(void)
{
  return "stm8-conventions_typearg_unsigned_int_typeret_unsigned_char_convention___cosmic";
}
