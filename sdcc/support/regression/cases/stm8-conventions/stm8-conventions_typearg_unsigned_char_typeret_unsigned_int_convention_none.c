...
#ifdef __SDCC_stm8
unsigned int f(unsigned char i) 
{
	return ++i;
}

unsigned int g(unsigned char i, unsigned int j) 
{
	return i + j;
}

unsigned int h(unsigned char i, unsigned int j, unsigned long long k) 
{
	return i + j + k;
}
...
unsigned int f_wrap_(unsigned char i) 
{
	return f(i);
}
#endif
...
__getSuiteName(void)
{
  return "stm8-conventions_typearg_unsigned_char_typeret_unsigned_int_convention_none";
}
