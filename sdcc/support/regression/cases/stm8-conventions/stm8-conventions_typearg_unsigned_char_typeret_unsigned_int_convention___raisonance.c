...
#ifdef __SDCC_stm8
unsigned int f(unsigned char i) __raisonance
{
	return ++i;
}

unsigned int g(unsigned char i, unsigned int j) __raisonance
{
	return i + j;
}

unsigned int h(unsigned char i, unsigned int j, unsigned long long k) __raisonance
{
	return i + j + k;
}
...
unsigned int f_wrap___raisonance(unsigned char i) __raisonance
{
	return f(i);
}
#endif
...
__getSuiteName(void)
{
  return "stm8-conventions_typearg_unsigned_char_typeret_unsigned_int_convention___raisonance";
}
