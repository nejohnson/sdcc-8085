...
unsigned char g(unsigned char i, unsigned int j) 
{
	return i + j;
}

unsigned char h(unsigned char i, unsigned int j, unsigned long long k) 
{
	return i + j + k;
}
...
unsigned char f_wrap_(unsigned char i) 
{
	return f(i);
}
#endif
...
testCallingConvention(void)
{
  ...
  ASSERT(f_wrap_(23) == (unsigned char)24);
#endif
}
...
__getSuiteName(void)
{
  return "stm8-conventions_typearg_unsigned_char_typeret_unsigned_char_convention_none";
}
