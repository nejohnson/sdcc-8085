...
#ifdef __SDCC_stm8
unsigned int f(unsigned char i) __cosmic
{
	return ++i;
}

unsigned int g(unsigned char i, unsigned int j) __cosmic
{
	return i + j;
}

unsigned int h(unsigned char i, unsigned int j, unsigned long long k) __cosmic
{
	return i + j + k;
}

unsigned int f_wrap_sdcc(unsigned char i)
{
	return f(i);
}

unsigned int f_wrap___cosmic(unsigned char i) __cosmic
{
	return f(i);
}
#endif
...
testCallingConvention(void)
{
  ...
  ASSERT(f_wrap___cosmic(23) == (unsigned int)24);
#endif
}
...
__getSuiteName(void)
{
  return "stm8-conventions_typearg_unsigned_char_typeret_unsigned_int_convention___cosmic";
}
