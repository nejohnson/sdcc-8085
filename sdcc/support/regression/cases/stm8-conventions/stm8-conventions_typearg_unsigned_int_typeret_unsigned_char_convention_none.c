...
unsigned char g(unsigned int i, unsigned int j) 
{
	return i + j;
}

unsigned char h(unsigned int i, unsigned int j, unsigned long long k) 
{
	return i + j + k;
}
...
unsigned char f_wrap_(unsigned int i) 
{
	return f(i);
}
#endif
...
__getSuiteName(void)
{
  return "stm8-conventions_typearg_unsigned_int_typeret_unsigned_char_convention_none";
}
