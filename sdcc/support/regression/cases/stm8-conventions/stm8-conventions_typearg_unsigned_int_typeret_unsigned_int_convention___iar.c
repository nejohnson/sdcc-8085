...
#ifdef __SDCC_stm8
unsigned int f(unsigned int i) __iar
{
	return ++i;
}

unsigned int g(unsigned int i, unsigned int j) __iar
{
	return i + j;
}

unsigned int h(unsigned int i, unsigned int j, unsigned long long k) __iar
{
	return i + j + k;
}
...
unsigned int f_wrap___iar(unsigned int i) __iar
{
	return f(i);
}
#endif
...
__getSuiteName(void)
{
  return "stm8-conventions_typearg_unsigned_int_typeret_unsigned_int_convention___iar";
}
