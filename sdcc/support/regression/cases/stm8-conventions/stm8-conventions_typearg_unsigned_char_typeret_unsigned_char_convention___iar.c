...
#ifdef __SDCC_stm8
unsigned char f(unsigned char i) __iar
{
	return ++i;
}

unsigned char g(unsigned char i, unsigned int j) __iar
{
	return i + j;
}

unsigned char h(unsigned char i, unsigned int j, unsigned long long k) __iar
{
	return i + j + k;
}
...
unsigned char f_wrap___iar(unsigned char i) __iar
{
	return f(i);
}
#endif
...
__getSuiteName(void)
{
  return "stm8-conventions_typearg_unsigned_char_typeret_unsigned_char_convention___iar";
}
