/*
   ...
   The test is based on the reference implementations of Ascon-AEAD128
   by Christoph Dobraunig and Martin Schläffer of the variants from the
...
// Implementation does an ugly cast and uses implementation-defined endiannness check instead of standard one
...
#if defined(__SDCC_mcs51) || defined(__SDCC_pdk14) || defined(__SDCC_pdk14) || defined(__SDCC_pdk15) || defined(__SDCC_mos6502) || defined(__SDCC_mos65c02)
#undef OP_encrypt
#undef IMPL_opt8_mylowsize
#endif
...
// Bug #3919 - fails test
#if defined(__SDCC_ds390) && defined(IMPL_ref) && defined(OP_encrypt)
#undef OP_encrypt
#endif

// Bug #3920 - fails test
#if defined(IMPL_opt32_lowsize) && __STDC_ENDIAN_NATIVE__ == __STDC_ENDIAN_BIG__
#undef OP_encrypt
#undef OP_decrypt
#endif

// Bug #3920 - fails test
#if defined(IMPL_opt8) && __STDC_ENDIAN_NATIVE__ == __STDC_ENDIAN_BIG__
#undef OP_encrypt
#undef OP_decrypt
#endif

// Bug #3920 - fails test
#if defined(IMPL_bi8) && (__STDC_ENDIAN_NATIVE__ == __STDC_ENDIAN_BIG__ || defined (__SDCC))
#undef OP_encrypt
#undef OP_decrypt
#endif

// Bug #3920 - fails test
#if defined(IMPL_opt8_lowsize) && __STDC_ENDIAN_NATIVE__ == __STDC_ENDIAN_BIG__
#undef OP_encrypt
#undef OP_decrypt
#endif

// Bug #3920 - fails test
#if defined(IMPL_opt8_mylowsize) && __STDC_ENDIAN_NATIVE__ == __STDC_ENDIAN_BIG__
#undef OP_encrypt
#undef OP_decrypt
#endif
...
#ifdef OP_encrypt
int ascon_aead_encrypt(uint8_t* t, uint8_t* c, const uint8_t* m, size_t mlen,
                       const uint8_t* ad, size_t adlen, const uint8_t* npub,
                       const uint8_t* k) {
  const int nr = (ASCON_AEAD_RATE == 8) ? 6 : 8;
  ascon_state_t s;
...
#ifdef OP_decrypt
int ascon_aead_decrypt(uint8_t* m, const uint8_t* t, const uint8_t* c,
                       uint64_t clen, const uint8_t* ad, size_t adlen,
                       const uint8_t* npub, const uint8_t* k) {
  const int nr = (ASCON_AEAD_RATE == 8) ? 6 : 8;
  ascon_state_t s;
  ascon_key_t key;
...
int crypto_aead_decrypt(unsigned char* m, unsigned long long* mlen,
  ...
  /* set plaintext size */
  *mlen = clen - CRYPTO_ABYTES;
  const uint8_t* t = (const uint8_t*)c + *mlen;
  print("decrypt\n");
  printbytes("k", k, CRYPTO_KEYBYTES);
  printbytes("n", npub, CRYPTO_NPUBBYTES);
  printbytes("a", ad, adlen);
...
// Test vector 333 from reference implementation.
const unsigned char key[CRYPTO_KEYBYTES] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};
const unsigned char nonce[CRYPTO_NPUBBYTES] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F};
const unsigned char pt[] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29};
const unsigned long long ptlen = 10;
const unsigned char ad[] = {0x30, 0x31};
const unsigned long long adlen = 2;
const unsigned char ct[] = {0x30, 0xFC, 0xEF, 0xAD, 0x28, 0x27, 0x5D, 0xF1, 0xA3, 0x1E, 0x58, 0xD5, 0xD7, 0x16, 0xAE, 0x8F, 0xFF, 0x42, 0xF8, 0xF1, 0x76, 0x1C, 0x91, 0xCB, 0xE3, 0xD8};
const unsigned long long ctlen = 26;
...
{
#ifdef OP_encrypt
  ASSERT(!crypto_aead_encrypt(buf, &blen, pt, ptlen, ad, adlen, NULL, nonce, key));
  ASSERT(blen == ctlen);
  ASSERT(!memcmp(buf, ct, blen));
#endif
#ifdef OP_decrypt
  ASSERT(!crypto_aead_decrypt(buf, &blen, NULL, ct, ctlen, ad, adlen, nonce, key));
  ASSERT(blen == ptlen);
  ASSERT(!memcmp(buf, pt, blen));
#endif
}
...
__getSuiteName(void)
{
  return "asconaead128_op_encrypt_impl_opt8_mylowsize";
}
