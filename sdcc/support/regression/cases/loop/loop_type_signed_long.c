...
void loop8(unsigned char *a, signed long n)
{
...
{
	for (signed long i = (1ul << 20); i < (1ul << 20) + 1; i++)
		a[i - (1ul << 20)] = 1;
}

void modify1(signed long *p)
{
	*p = 17;
}

void modify2(signed long *p)
{
	*p = (1ul << 30);
}
...
{
	for (signed long i = (1ul << 28); i < (1ul << 30); i++)
	{
		modify1(&i);
		a[i] = 17;
		modify2(&i);
...
void jump_func(jmp_buf *jp, signed long i)
{
	ASSERT (i == (1ul << 29));
	longjmp (*jp, 0);
}
...
void jump(unsigned char *a)
{
	...
	for (signed long i = (1ul << 29); i < (1ul << 30); i++)
	{
		jump_func(&j, i);
		a[i] = 14;
	}
