...
unsigned short ror(unsigned short v, unsigned int c)
{
  c %= sizeof(unsigned short) * 8;
  return(v >> c) | (v << (sizeof(unsigned short) * 8 - c));
}
...
__runSuite(void)
{
  __prints("Running testRotateRight\n");
  testRotateRight();
}
...
__getSuiteName(void)
{
  return "rotate_right_type_unsigned_short";
}
